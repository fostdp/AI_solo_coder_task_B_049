...
public:
    ...
    void initialize(int sample_rate = 1000, int window_size = 500, int num_threads = 0);

    TechniqueAnalysisResult analyze(const std::vector<double>& emg_signal, uint64_t timestamp);

    std::future<TechniqueAnalysisResult> analyze_async(
    ...
    bool train(const std::vector<std::vector<double>>& samples,
               ...
               int batch_size = 16,
               double* loss_before = nullptr,
               double* loss_after = nullptr);
