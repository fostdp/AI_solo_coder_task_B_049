...
void RLAdapter::initialize(double actor_lr,
                           double critic_lr,
...
void RLAdapter::init_default_actions() {
    actions_.clear();
...
std::string RLAdapter::discretize_deqi(double deqi) const {
...
std::string RLAdapter::discretize_pain(double pain) const {
...
std::string RLAdapter::discretize_duration(double duration) const {
...
std::string RLAdapter::state_to_key(const AcupunctureState& state) const {
...
void RLAdapter::ensure_state_exists(const std::string& state_key) {
...
std::vector<double> RLAdapter::get_action_preferences(const std::string& state_key) {
    ensure_state_exists(state_key);
    return actor_weights_[state_key];
}

std::vector<double> RLAdapter::compute_softmax(const std::vector<double>& preferences) const {
...
int RLAdapter::sample_action_from_probabilities(const std::vector<double>& probs) {
...
int RLAdapter::select_best_action(const std::vector<double>& probs) const {
    int best_idx = 0;
    double best = probs[0];
...
RLResult RLAdapter::recommend_action(const AcupunctureState& state) {
...
double RLAdapter::normalize_reward(double raw_reward) const {
...
double RLAdapter::compute_reward(const AcupunctureState& state,
...
double RLAdapter::compute_td_error(double reward, const std::string& state_key,
...
void RLAdapter::update_actor(const std::string& state_key, int action_taken, double td_error) {
    ensure_state_exists(state_key);
...
void RLAdapter::update_critic(const std::string& state_key, double td_error) {
    ensure_state_exists(state_key);
    critic_values_[state_key] += critic_learning_rate_ * td_error;
}

void RLAdapter::update_q_table(const TrainingEpisode& episode) {
...
void RLAdapter::record_result(const AcupunctureState& state,
                              const AcupunctureAction& action,
                              double reward,
...
void RLAdapter::decay_exploration() {
...
std::vector<AcupunctureAction> RLAdapter::get_all_actions() const {
    return actions_;
}

void RLAdapter::reset_q_table() {
...
AcupunctureAction RLAdapter::action_from_index(int index) {
    RLAdapter dummy;
...
int RLAdapter::action_to_index(const AcupunctureAction& action) {
    return action.action_index;
}

void RLAdapter::save_model(const std::string& path) const {
    std::ofstream f(path);
    if (!f.is_open()) return;
...
void RLAdapter::load_model(const std::string& path) {
    std::ifstream f(path);
    if (!f.is_open()) return;
